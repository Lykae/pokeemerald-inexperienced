...
// Experience settings
#define B_EXP_CATCH                 GEN_1 // In Gen6+, Pokémon get experience from catching.
#define B_TRAINER_EXP_MULTIPLIER    GEN_LATEST // In Gen7+, trainer battles no longer give a 1.5 multiplier to EXP gain.
#define B_SPLIT_EXP                 GEN_1 // In Gen6+, all participating mon get full experience.
#define B_SCALED_EXP                GEN_1 // In Gen5 and Gen7+, experience is weighted by level difference.
#define B_UNEVOLVED_EXP_MULTIPLIER  GEN_1 // In Gen6+, if the Pokémon is at or past the level where it would be able to evolve, but it has not, it gets a ~1.2 multiplier to EXP gain. Only applies to Pokémon with EVO_LEVEL method.
...
#define B_FLAG_NO_BAG_USE           FLAG_NO_ITEMS_IN_BATTLE_0x020     // If this flag is set, the ability to use the bag in battle is disabled.
...
#define B_FLAG_TERA_ORB_CHARGED     FLAG_TERA_ORB_CHARGED_0x021     // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
